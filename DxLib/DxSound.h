...
extern	int		LoadSoundMemByMemImageBase_UseGParam( LOADSOUND_GPARAM *GParam, int CreateSoundHandle, int SoundHandle, void *FileImageBuffer, int ImageSize, int BufferNum, int UnionHandle = -1, int ASyncLoadFlag = FALSE, int ASyncThread = FALSE ) ;		// LoadSoundMemByMemImageBase のグローバル変数にアクセスしないバージョン
...
extern	int		LoadSoundMem2ByMemImage_UseGParam( LOADSOUND_GPARAM *GParam, void *FileImageBuffer1, int ImageSize1, void *FileImageBuffer2, int ImageSize2, int ASyncLoadFlag = FALSE ) ;																			// LoadSoundMem2ByMemImage のグローバル変数にアクセスしないバージョン
...
extern	int		Refresh3DSoundParam( int SoundHandle, int AlwaysFlag = FALSE ) ;										// ３Ｄサウンドのパラメータを更新する
