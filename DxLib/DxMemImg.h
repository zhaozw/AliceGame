...
extern	int		SetMemImgDrawArea( RECT *DrawArea ) ;																												// MEMIMG用の描画範囲を設定する
...
extern	void	BltMemImg( MEMIMG *DestImg, MEMIMG *SrcImg, RECT *SrcRect/* NULL:全体 */, int DestX, int DestY ) ;																	// (補助関数)MEMIMG 間でデータの転送を行う
...
extern	int		InitializeMemImg( MEMIMG *Img, int Width, int Height, int Pitch/*規定値:-1*/, int TransColor, int ColorType, int UsePalette, int UseAlpha, int AnalysisFlag = TRUE, void *UserImage = NULL ) ;		// メモリ画像を初期化する、構造体はゼロ初期化されている必要がある( 戻り値: -1=失敗  0=成功 )
extern	int		CheckValidMemImg( MEMIMG *Img ) ;																													// MEMIMG が有効かどうかを取得する
extern	void	TerminateMemImg( MEMIMG *Img ) ;																													// メモリ画像の後始末をする( 自前で初期化した場合は実行する必要なし( イメージ領域の解放だけなので ) )
extern	void	BltBaseImageToMemImg( BASEIMAGE *RgbImage, BASEIMAGE *AlphaImage, MEMIMG *MemImg, int SrcX, int SrcY, int Width, int Height, int DestX, int DestY, int UseTransColorConvAlpha = TRUE ) ;	// BASEIMAGE イメージから MEMIMG イメージに画像データを転送する( パレットも勝手に転送される )
extern	void	BltMemImgToBaseImage( BASEIMAGE *BaseImage, MEMIMG *MemImg, int SrcX, int SrcY, int Width, int Height, int DestX, int DestY, int TransColor = 0, int TransFlag = FALSE ) ;	// MEMIMG イメージから BASEIMAGE イメージに画像データを転送する
extern	void	ClearMemImg( MEMIMG *MemImg, RECT *FillArea = NULL, DWORD Color = 0 ) ;																				// イメージを初期化する
extern	void	DrawMemImg( MEMIMG *DestImg, MEMIMG *SrcImg, int DestX, int DestY, int TransFlag = TRUE, MEMIMG *BlendImg = NULL ) ;								// イメージを描画する
extern	void	DrawTurnMemImg( MEMIMG *DestImg, MEMIMG *SrcImg, int XTurnFlag, int YTurnFlag, int DestX, int DestY, int TransFlag = TRUE, MEMIMG *BlendImg = NULL ) ;
extern	void	DrawEnlargeMemImg( MEMIMG *DestImg, MEMIMG *SrcImg, RECT *DestRect, int TransFlag = TRUE, MEMIMG *BlendImg = NULL ) ;								// イメージを拡大描画する
extern	void	DrawEnlargeMemImg( MEMIMG *DestImg, MEMIMG *SrcImg, int x1, int y1, int x2, int y2, int TransFlag = TRUE, MEMIMG *BlendImg = NULL ) ;				// イメージを拡大描画する
extern	void 	DrawRotationMemImg( MEMIMG *DestImg, MEMIMG *SrcImg, int x, int y, float Angle, float EnlageRate, int TransFlag = TRUE, int TurnFlag = FALSE, MEMIMG *BlendImg = NULL ) ;	// イメージを回転描画する
extern	void	DrawBasicPolygonMemImg( MEMIMG *DestImg, MEMIMG *SrcImg, DX_POINTDATA *pos3, int TransFlag = TRUE, MEMIMG *BlendImg = NULL, int Color = 0xffffffff ) ;	// イメージを使用してポリゴンを描画する( ２Ｄ空間に画像を変形して描画するのが目的 )
extern	void	DrawTransformMemImg( MEMIMG *DestImg, MEMIMG *SrcImg, POINT *pos4, int TransFlag = TRUE, MEMIMG *BlendImg = NULL ) ;								// イメージを変形描画する
extern	void	DrawPolygonMemImg( MEMIMG *DestImg, MEMIMG *ZImg, MEMIMG *SrcImg, VERTEX_2D *Vertex3, int TransFlag = TRUE, MEMIMG *BlendImg = NULL, int ZWriteFlag = TRUE, int GouraudShadeMode = TRUE, int PerspectiveEnable = TRUE, int ScissorTest = TRUE ) ;	// イメージを使用してポリゴンを描画する( ３Ｄ空間に３Ｄポリゴンを描画するのが目的 )

extern	int		GetPixelColorMemImg( MEMIMG *MemImg, int x, int y ) ;																								// イメージ中の指定座標の色を取得
...
extern	void	DrawFillBoxMemImg( MEMIMG *DestImg, RECT *FillRect, int Color ) ;																					// イメージに中身のあるボックスを描画する
...
extern 	void	DrawPixelSetMemImg( MEMIMG *DestImg, POINTDATA *PointData, int Num ) ;																				// イメージに点の集合を描画する
extern	void	DrawLineSetMemImg( MEMIMG *DestImg, LINEDATA *LineData, int Num ) ;																					// イメージに線の集合を描画する
