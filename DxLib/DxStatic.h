...
extern	int			NS_NetWorkSend(			int NetHandle, void *Buffer, int Length ) ;														// データを送信する
...
extern	int			NS_NetWorkSendUDP(			int NetUDPHandle, IPDATA       SendIP, int SendPort,  void *Buffer, int Length ) ;				// UDPを使用した通信で指定のＩＰにデータを送信する、Length は最大65507、SendPort を -1 にすると MakeUDPSocket に RecvPort で渡したポートが使用されます( 戻り値  0以上;送信できたデータサイズ  -1:エラー  -2:送信データが大きすぎる  -3:送信準備ができていない  )
extern	int			NS_NetWorkSendUDP_IPv6(	int NetUDPHandle, IPDATA_IPv6  SendIP, int SendPort,  void *Buffer, int Length ) ;				// UDPを使用した通信で指定のＩＰにデータを送信する、Length は最大65507、SendPort を -1 にすると MakeUDPSocket に RecvPort で渡したポートが使用されます( 戻り値  0以上;送信できたデータサイズ  -1:エラー  -2:送信データが大きすぎる  -3:送信準備ができていない  )( IPv6版 )
...
extern	TCHAR*		NS_GetResourceIDString(	int ResourceID ) ;																						// リソースＩＤからリソースＩＤ文字列を得る 
...
extern	int			NS_GetWindowSize(					int *Width, int *Height ) ;								// ウインドウモードのウインドウののクライアント領域のサイズを取得する
...
extern	int			NS_LoadPauseGraphFromMem(					void *MemImage , int MemImageSize ) ;												// アクティブウインドウが他のソフトに移っている際に表示する画像のロード(NULL で解除)
...
// ウインドウ描画領域設定系関数
extern	HRGN		NS_CreateRgnFromGraph(			int Width , int Height , void *MaskData , int Pitch , int Byte ) ;				// 任意のグラフィックからRGNハンドルを作成する
...
extern	IMEINPUTDATA* NS_GetIMEInputData(			void ) ;																														// IMEで入力中の文字列の情報を取得する
...
extern	int			NS_FileRead_size(				const TCHAR *FilePath ) ;										// ファイルのサイズを得る
extern	int			NS_FileRead_close(				int FileHandle ) ;												// ファイルを閉じる
extern	int			NS_FileRead_tell(				int FileHandle ) ;												// ファイルポインタの現在位置を得る
extern	int			NS_FileRead_seek(				int FileHandle , int Offset , int Origin ) ;					// ファイルポインタの位置を変更する
...
extern	int			NS_ChangeStreamFunction(		STREAMDATASHREDTYPE2 *StreamThread ) ;							// ＤＸライブラリでストリームデータアクセスに使用する関数を変更する
...
// グラフィックハンドルへの画像転送関数
extern	int			NS_BltBmpToGraph(					COLORDATA *BmpColorData, HBITMAP RgbBmp, HBITMAP AlphaBmp, int CopyPointX, int CopyPointY,                                                                                 int GrHandle ) ;						// ＢＭＰの内容をグラフィックハンドルに転送
extern	int			NS_BltBmpToDivGraph(				COLORDATA *BmpColorData, HBITMAP RgbBmp, HBITMAP AlphaBmp, int AllNum, int XNum, int YNum, int Width, int Height,                                                          int *GrHandle, int ReverseFlag ) ;	// ＢＭＰの内容を分割作成したグラフィックハンドルたちに転送
extern	int			NS_BltBmpOrGraphImageToGraph(		COLORDATA *BmpColorData, HBITMAP RgbBmp, HBITMAP AlphaBmp, int BmpFlag, BASEIMAGE *RgbImage, BASEIMAGE *AlphaImage, int CopyPointX, int CopyPointY,                        int GrHandle ) ;						// ＢＭＰ か BASEIMAGE をグラフィックハンドルに転送
extern	int			NS_BltBmpOrGraphImageToGraph2(		COLORDATA *BmpColorData, HBITMAP RgbBmp, HBITMAP AlphaBmp, int BmpFlag, BASEIMAGE *RgbImage, BASEIMAGE *AlphaImage, RECT *SrcRect, int DestX, int DestY,                   int GrHandle ) ;						// ＢＭＰ か BASEIMAGE の指定の領域をグラフィックハンドルに転送
extern	int			NS_BltBmpOrGraphImageToDivGraph(	COLORDATA *BmpColorData, HBITMAP RgbBmp, HBITMAP AlphaBmp, int BmpFlag, BASEIMAGE *RgbImage, BASEIMAGE *AlphaImage, int AllNum, int XNum, int YNum, int Width, int Height, int *GrHandle, int ReverseFlag ) ;	// ＢＭＰ か BASEIMAGE を分割作成したグラフィックハンドルたちに転送
...
extern	int			NS_CreateGraphFromMem(				void *RGBFileImage, int RGBFileImageSize,               void *AlphaFileImage = NULL , int AlphaFileImageSize = 0 ,      int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// メモリ上のグラフィックイメージからグラフィックハンドルを作成する
extern	int			NS_ReCreateGraphFromMem(           void *RGBFileImage, int RGBFileImageSize, int GrHandle, void *AlphaFileImage = NULL , int AlphaFileImageSize = 0 ,      int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// メモリ上のグラフィックイメージから既存のグラフィックハンドルにデータを転送する
extern	int			NS_CreateDivGraphFromMem(          void *RGBFileImage, int RGBFileImageSize, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf,         int TextureFlag = TRUE , int ReverseFlag = FALSE , void *AlphaFileImage = NULL , int AlphaFileImageSize = 0 ) ;		// メモリ上のグラフィックイメージから分割グラフィックハンドルを作成する
extern	int			NS_ReCreateDivGraphFromMem(        void *RGBFileImage, int RGBFileImageSize, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf,         int TextureFlag = TRUE , int ReverseFlag = FALSE , void *AlphaFileImage = NULL , int AlphaFileImageSize = 0 ) ;		// メモリ上のグラフィックイメージから既存の分割グラフィックハンドルにデータを転送する
extern	int			NS_CreateGraphFromBmp(             BITMAPINFO *RGBBmpInfo, void *RGBBmpImage,               BITMAPINFO *AlphaBmpInfo = NULL , void *AlphaBmpImage = NULL , int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// ビットマップデータからグラフィックハンドルを作成する
extern	int			NS_ReCreateGraphFromBmp(           BITMAPINFO *RGBBmpInfo, void *RGBBmpImage, int GrHandle, BITMAPINFO *AlphaBmpInfo = NULL , void *AlphaBmpImage = NULL , int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// ビットマップデータから既存のグラフィックハンドルにデータを転送する
extern	int			NS_CreateDivGraphFromBmp(          BITMAPINFO *RGBBmpInfo, void *RGBBmpImage, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf,        int TextureFlag = TRUE , int ReverseFlag = FALSE , BITMAPINFO *AlphaBmpInfo = NULL , void *AlphaBmpImage = NULL ) ;	// ビットマップデータから分割グラフィックハンドルを作成する
extern	int			NS_ReCreateDivGraphFromBmp(        BITMAPINFO *RGBBmpInfo, void *RGBBmpImage, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf,        int TextureFlag = TRUE , int ReverseFlag = FALSE , BITMAPINFO *AlphaBmpInfo = NULL , void *AlphaBmpImage = NULL ) ;	// ビットマップデータから既存の分割グラフィックハンドルにデータを転送する
extern	int			NS_CreateDXGraph(					BASEIMAGE *RgbImage, BASEIMAGE *AlphaImage,																				int TextureFlag ) ;																									// GraphImage データからサイズを割り出し、それに合ったグラフィックハンドルを作成する
extern	int			NS_CreateGraphFromGraphImage(      BASEIMAGE *RGBImage,                                                                                                    int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// GraphImageデータからグラフィックハンドルを作成する
extern	int			NS_CreateGraphFromGraphImage(      BASEIMAGE *RGBImage, BASEIMAGE *AlphaImage,                                                                             int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// GraphImageデータからグラフィックハンドルを作成する
extern	int			NS_ReCreateGraphFromGraphImage(    BASEIMAGE *RGBImage,                        int GrHandle,                                                               int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// GraphImageデータから既存のグラフィックハンドルにデータを転送する
extern	int			NS_ReCreateGraphFromGraphImage(    BASEIMAGE *RGBImage, BASEIMAGE *AlphaImage, int GrHandle,                                                               int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// GraphImageデータから既存のグラフィックハンドルにデータを転送する
...
extern	int			NS_CreateDivGraphFromGraphImage(   BASEIMAGE *RGBImage, BASEIMAGE *AlphaImage, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf,       int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// GraphImageデータから分割グラフィックハンドルを作成する
extern	int			NS_ReCreateDivGraphFromGraphImage( BASEIMAGE *RGBImage,                        int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf,       int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// GraphImageデータから既存の分割グラフィックハンドルにデータを転送する
extern	int			NS_ReCreateDivGraphFromGraphImage( BASEIMAGE *RGBImage, BASEIMAGE *AlphaImage, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf,       int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																// GraphImageデータから既存の分割グラフィックハンドルにデータを転送する
extern	int			NS_CreateGraph(                    int Width, int Height, int Pitch, void *RGBImage, void *AlphaImage = NULL , int GrHandle = -1 ) ;																																			// メモリ上のグラフィックデータからグラフィックハンドルを作成する
extern	int			NS_CreateDivGraph(                 int Width, int Height, int Pitch, void *RGBImage, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf, void *AlphaImage = NULL ) ;																							// メモリ上のグラフィックデータから分割グラフィックハンドルを作成する
extern	int			NS_ReCreateGraph(                  int Width, int Height, int Pitch, void *RGBImage, int GrHandle, void *AlphaImage = NULL ) ;																																					// メモリ上のグラフィックデータからグラフィックハンドルを再作成する
...
extern	int			NS_CreateGraphFromBaseImage(       BASEIMAGE *BaseImage ) ;																											// 基本イメージデータからグラフィックハンドルを作成する
extern	int			NS_CreateGraphFromRectBaseImage(   BASEIMAGE *BaseImage, int x, int y, int SizeX, int SizeY ) ;																		// 基本イメージの指定の領域を使ってグラフィックハンドルを作成する
extern	int			NS_ReCreateGraphFromBaseImage(     BASEIMAGE *BaseImage,                                     int GrHandle ) ;															// 基本イメージデータから既存のグラフィックハンドルに画像データを転送する
extern	int			NS_ReCreateGraphFromRectBaseImage( BASEIMAGE *BaseImage, int x, int y, int SizeX, int SizeY, int GrHandle ) ;															// 基本イメージの指定の領域を使って既存のグラフィックハンドルに画像データを転送する
...
extern	int			NS_ReloadDivGraph(					const TCHAR *FileName, int AllNum, int XNum, int YNum, int XSize, int YSize, int *HandleBuf, int ReverseFlag = FALSE ) ;			// 画像への画像データの分割読み込み
extern	int			NS_ReloadReverseGraph(				const TCHAR *FileName, int GrHandle ) ;																								// 画像への画像データの読み込み
extern	int			NS_ReloadReverseDivGraph(			const TCHAR *FileName, int AllNum, int XNum, int YNum, int XSize, int YSize, int *HandleBuf ) ;										// 画像への画像データの分割読み込み
...
// 画像情報関係関数
extern	DWORD*		NS_GetFullColorImage(				int GrHandle ) ;																		// 指定の画像のＡＲＧＢ８のフルカラーイメージを取得する
...
extern	int			NS_SetUseGraphZBuffer(				int GrHandle, int UseFlag ) ;															// グラフィック専用のＺバッファを持つかどうかを設定する( 描画可能画像のみ可能 )
...
extern	COLORDATA*	NS_GetTexColorData(				int AlphaCh, int AlphaTest, int ColorBitDepth, int DrawValid = FALSE ) ;				// カラーデータを得る
extern	COLORDATA*	NS_GetTexColorData(				IMAGEFORMATDESC *Format ) ;																// フォーマットに基づいたカラーデータを得る
extern	COLORDATA*	NS_GetTexColorData(				int FormatIndex ) ;																		// 指定のフォーマットインデックスのカラーデータを得る
...
extern	int			NS_DrawCircle(     int x, int y, int r,                                            int Color, int FillFlag = TRUE ) ;									// 円を描く
extern	int			NS_DrawOval(       int x, int y, int rx, int ry,                                   int Color, int FillFlag ) ;											// 楕円を描く
...
extern	int			NS_DrawPixelSet(   POINTDATA *PointData, int Num ) ;																									// 点の集合を描く
extern	int			NS_DrawLineSet(    LINEDATA *LineData,   int Num ) ;																									// 線の集合を描く
...
extern	int			NS_DrawCapsule3D(  VECTOR Pos1, VECTOR Pos2,        float r, int DivNum, int DifColor, int SpcColor, int FillFlag ) ;									// ３Ｄのカプセルを描画する
...
extern	int			NS_DrawChipMap(              int Sx, int Sy, int XNum, int YNum, int *MapData, int ChipTypeNum, int MapDataPitch, int *ChipGrHandle, int TransFlag ) ;																											// チップグラフィックを使ったマップ描画
extern	int			NS_DrawChipMap(              int MapWidth, int MapHeight,        int *MapData, int ChipTypeNum,                   int *ChipGrHandle, int TransFlag, int MapDrawPointX, int MapDrawPointY, int MapDrawWidth, int MapDrawHeight, int ScreenX, int ScreenY ) ;	// チップグラフィックを使ったマップ描画
...
extern	int			NS_DrawCircleGauge(          int CenterX, int CenterY, double Percent, int GrHandle ) ;																						// 円グラフ的な描画を行う( GrHandle の画像の上下左右の端は透過色にしておく必要があります )
...
extern	int			NS_DrawPolygon(                             VERTEX    *Vertex, int PolygonNum,                                                                                              int GrHandle, int TransFlag, int UVScaling = FALSE ) ;		// ２Ｄポリゴンを描画する( Vertex:三角形を形成する頂点配列の先頭アドレス( 頂点の数はポリゴンの数×３ )  PolygonNum:描画するポリゴンの数  GrHandle:使用するグラフィックハンドル  TransFlag:透過色処理を行うかどうか( TRUE:行う  FALSE:行わない )  UVScaling:基本FALSEでOK )
extern	int			NS_DrawPolygon2D(                           VERTEX2D  *Vertex, int PolygonNum,                                                                                              int GrHandle, int TransFlag ) ;							// ２Ｄポリゴンを描画する
extern	int			NS_DrawPolygon3D(                           VERTEX3D  *Vertex, int PolygonNum,                                                                                              int GrHandle, int TransFlag ) ;							// ３Ｄポリゴンを描画する
extern	int			NS_DrawPolygonIndexed2D(                    VERTEX2D  *Vertex, int VertexNum, unsigned short *Indices, int PolygonNum,                                                      int GrHandle, int TransFlag ) ;							// ２Ｄポリゴンを描画する( 頂点インデックスを使用 )
extern	int			NS_DrawPolygonIndexed3D(                    VERTEX3D  *Vertex, int VertexNum, unsigned short *Indices, int PolygonNum,                                                      int GrHandle, int TransFlag ) ;							// ３Ｄポリゴンを描画する( 頂点インデックスを使用 )
extern	int			NS_DrawPolygonIndexed3DBase(                VERTEX_3D *Vertex, int VertexNum, unsigned short *Indices, int IndexNum,   int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;							// ３Ｄポリゴンを描画する( 頂点インデックスを使用 )( 旧バージョン用 )
extern	int			NS_DrawPolygon3DBase(                       VERTEX_3D *Vertex, int VertexNum,                                          int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;							// ３Ｄポリゴンを描画する( 旧バージョン用 )
extern	int			NS_DrawPolygon3D(                           VERTEX_3D *Vertex, int PolygonNum,                                                                                              int GrHandle, int TransFlag ) ;							// ３Ｄポリゴンを描画する( 旧バージョン用 )

extern	int			NS_DrawPolygonBase(                         VERTEX    *Vertex, int VertexNum,                                          int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag, int UVScaling = FALSE ) ;		// ２Ｄプリミティブを描画する
extern	int			NS_DrawPrimitive2D(                         VERTEX2D  *Vertex, int VertexNum,                                          int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;							// ２Ｄプリミティブを描画する
extern	int			NS_DrawPrimitive3D(                         VERTEX3D  *Vertex, int VertexNum,                                          int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;							// ３Ｄプリミティブを描画する
extern	int			NS_DrawPrimitiveIndexed2D(                  VERTEX2D  *Vertex, int VertexNum, unsigned short *Indices, int IndexNum,   int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;							// ２Ｄプリミティブを描画する(頂点インデックス使用)
extern	int			NS_DrawPrimitiveIndexed3D(                  VERTEX3D  *Vertex, int VertexNum, unsigned short *Indices, int IndexNum,   int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;							// ３Ｄプリミティブを描画する(頂点インデックス使用)
...
extern	int			NS_SetTransformToWorld(				MATRIX *Matrix ) ;												// ワールド変換用行列をセットする
extern	int			NS_GetTransformToWorldMatrix(			MATRIX *MatBuf ) ;												// ワールド行列を取得する
extern	int			NS_SetTransformToView(					MATRIX *Matrix ) ;												// ビュー変換用行列をセットする
extern	int			NS_GetTransformToViewMatrix(			MATRIX *MatBuf ) ;												// ビュー行列を取得する
extern	int			NS_SetTransformToProjection(			MATRIX *Matrix ) ;												// 投影変換用行列をセットする
extern	int			NS_GetTransformToProjectionMatrix(		MATRIX *MatBuf ) ;												// 射影行列を取得する
extern	int			NS_SetTransformToViewport(				MATRIX *Matrix ) ;												// ビューポート行列をセットする
...
extern	int			NS_SetDefTransformMatrix(				void ) ;														// デフォルトの変換行列をセットする
...
extern	int				NS_ClearDrawScreen(					        RECT *ClearRect = NULL ) ;														// 画面の状態を初期化する
extern	int				NS_ClearDrawScreenZBuffer(						RECT *ClearRect = NULL ) ;														// 画面のＺバッファの状態を初期化する
...
extern	COLORDATA*		NS_GetDispColorData(							void ) ;																		// ディスプレーのカラーデータポインタを得る
...
extern	int			NS_SetWindowDrawRect(							RECT *DrawRect ) ;								// 通常使用しない
...
extern	int			NS_SetDDrawUseGuid(							GUID *Guid ) ;									// ＤｉｒｅｃｔＤｒａｗが使用するＧＵＤＩを設定する
extern	void*		NS_GetUseDDrawObj(								void ) ;										// 現在使用しているＤｉｒｅｃｔＤｒａｗオブジェクトのアドレスを取得する( 戻り値を IDirectDraw7 * にキャストして下さい )
extern	GUID*		NS_GetDirectDrawDeviceGUID(					int Number ) ;									// 有効な DirectDraw デバイスの GUID を取得する
...
extern	DX_DIRECT3DDEVICE9* NS_GetUseDirect3DDevice9(				void ) ;										// 使用中のＤｉｒｅｃｔ３ＤＤｅｖｉｃｅ９オブジェクトを得る
extern	DX_DIRECT3DSURFACE9 *NS_GetUseDirect3D9BackBufferSurface(	void ) ;										// 使用中のバックバッファのDirect3DSurface9オブジェクトを取得する
#endif // __WINDOWS__
...
extern	int			NS_SetVertexBufferData(	int SetIndex, void *VertexData, int VertexNum, int VertexBufHandle ) ;					// 頂点バッファに頂点データを転送する
...
extern	int			NS_SetIndexBufferData(		int SetIndex, void *IndexData, int IndexNum, int IndexBufHandle ) ;						// インデックスバッファにインデックスデータを転送する
...
extern	int			NS_LoadVertexShaderFromMem(	void *ImageAddress, int ImageSize ) ;					// メモリ空間上に存在する頂点シェーダーバイナリから頂点シェーダーハンドルを作成する( 戻り値 -1:エラー  -1以外:シェーダーハンドル )
extern	int			NS_LoadPixelShaderFromMem(		void *ImageAddress, int ImageSize ) ;					// メモリ空間上に存在するピクセルシェーダーバイナリからピクセルシェーダーハンドルを作成する( 戻り値 -1:エラー  -1以外:シェーダーハンドル )
...
extern	int			NS_SetVSConstSFArray(    int ConstantIndex, float  *ParamArray, int ParamNum ) ;		// 頂点シェーダーの float 型定数を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetVSConstFArray(     int ConstantIndex, FLOAT4 *ParamArray, int ParamNum ) ;		// 頂点シェーダーの float 型定数を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetVSConstFMtxArray(  int ConstantIndex, MATRIX *ParamArray, int ParamNum ) ;		// 頂点シェーダーの float 型定数に行列を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetVSConstFMtxTArray( int ConstantIndex, MATRIX *ParamArray, int ParamNum ) ;		// 頂点シェーダーの float 型定数に転置した行列を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetVSConstSIArray(    int ConstantIndex, int    *ParamArray, int ParamNum ) ;		// 頂点シェーダーの int 型定数を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetVSConstIArray(     int ConstantIndex, INT4   *ParamArray, int ParamNum ) ;		// 頂点シェーダーの int 型定数を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetVSConstBArray(     int ConstantIndex, BOOL   *ParamArray, int ParamNum ) ;		// 頂点シェーダーの BOOL 型定数を設定する( 配列を使って連番インデックスに一度に設定 )
...
extern	int			NS_SetPSConstSFArray(    int ConstantIndex, float  *ParamArray, int ParamNum ) ;		// ピクセルシェーダーの float 型定数を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetPSConstFArray(     int ConstantIndex, FLOAT4 *ParamArray, int ParamNum ) ;		// ピクセルシェーダーの float 型定数を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetPSConstFMtxArray(  int ConstantIndex, MATRIX *ParamArray, int ParamNum ) ;		// ピクセルシェーダーの float 型定数に行列を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetPSConstFMtxTArray( int ConstantIndex, MATRIX *ParamArray, int ParamNum ) ;		// ピクセルシェーダーの float 型定数に転置した行列を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetPSConstSIArray(    int ConstantIndex, int    *ParamArray, int ParamNum ) ;		// ピクセルシェーダーの int 型定数を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetPSConstIArray(     int ConstantIndex, INT4   *ParamArray, int ParamNum ) ;		// ピクセルシェーダーの int 型定数を設定する( 配列を使って連番インデックスに一度に設定 )
extern	int			NS_SetPSConstBArray(     int ConstantIndex, BOOL   *ParamArray, int ParamNum ) ;		// ピクセルシェーダーの BOOL 型定数を設定する( 配列を使って連番インデックスに一度に設定 )
...
extern	int			NS_SetRenderTargetToShader( int TargetIndex, int DrawScreen ) ;						// シェーダー描画での描画先を設定する
...
extern	int			NS_CalcPolygonIndexedBinormalAndTangentsToShader( VERTEX3DSHADER *Vertex, int VertexNum, unsigned short *Indices, int PolygonNum ) ;		// ポリゴンの頂点の接線と従法線をＵＶ座標から計算してセットする(インデックス)

extern	int			NS_DrawPolygon2DToShader(          VERTEX2DSHADER *Vertex, int PolygonNum ) ;																												// シェーダーを使って２Ｄポリゴンを描画する
extern	int			NS_DrawPolygon3DToShader(          VERTEX3DSHADER *Vertex, int PolygonNum ) ;																												// シェーダーを使って３Ｄポリゴンを描画する
extern	int			NS_DrawPolygonIndexed2DToShader(   VERTEX2DSHADER *Vertex, int VertexNum, unsigned short *Indices, int PolygonNum ) ;																		// シェーダーを使って２Ｄポリゴンを描画する(インデックス)
extern	int			NS_DrawPolygonIndexed3DToShader(   VERTEX3DSHADER *Vertex, int VertexNum, unsigned short *Indices, int PolygonNum ) ;																		// シェーダーを使って３Ｄポリゴンを描画する(インデックス)
extern	int			NS_DrawPrimitive2DToShader(        VERTEX2DSHADER *Vertex, int VertexNum,                                        int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */ ) ;					// シェーダーを使って２Ｄプリミティブを描画する
extern	int			NS_DrawPrimitive3DToShader(        VERTEX3DSHADER *Vertex, int VertexNum,                                        int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */ ) ;					// シェーダーを使って３Ｄプリミティブを描画する
extern	int			NS_DrawPrimitiveIndexed2DToShader( VERTEX2DSHADER *Vertex, int VertexNum, unsigned short *Indices, int IndexNum, int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */ ) ;					// シェーダーを使って２Ｄプリミティブを描画する(インデックス)
extern	int			NS_DrawPrimitiveIndexed3DToShader( VERTEX3DSHADER *Vertex, int VertexNum, unsigned short *Indices, int IndexNum, int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */ ) ;					// シェーダーを使って３Ｄプリミティブを描画する(インデックス)
...
extern	BASEIMAGE*	NS_GetMovieBaseImageToGraph(		int GraphHandle ) ;														// ムービーの基本イメージデータを取得する
...
extern	int			NS_SetMovieRightImageAlphaFlag(	int Flag ) ;															// 動画ファイルの右半分の赤成分ををアルファ情報として扱うかどうかをセットする( TRUE:アルファ情報として扱う )
#endif
...
// 色情報取得用関数
extern	int			NS_GetTexFormatIndex(			IMAGEFORMATDESC *Format ) ;																									// テクスチャフォーマットのインデックスを得る
extern	int			NS_ColorKaiseki(				void *PixelData, COLORDATA* ColorData ) ;																					// 色ビット情報解析
#ifdef __WINDOWS__
...
extern	int			NS_DrawMaskToDirectData(			int x, int y, int Width, int Height, void *MaskData , int TransMode ) ;							// マスクのデータを直接セット
extern	int			NS_DrawFillMaskToDirectData(		int x1, int y1, int x2, int y2,  int Width, int Height, void *MaskData ) ;						// マスクのデータを直接マスク画面全体に描画する
...
extern	int			NS_SetDataToMask(					int Width, int Height, void *MaskData, int MaskHandle ) ;										// マスクのデータをマスクに転送する
...
extern	int			NS_EnumFontNameEx2(						TCHAR *NameBuffer, int NameBufferNum, TCHAR *EnumFontName, int CharSet = -1 /* DX_CHARSET_DEFAULT 等 */ ) ;	// 指定のフォント名のフォントを列挙する
...
extern	int			NS_SetDefaultFontState(                    const TCHAR *FontName, int Size, int Thick ) ;													// デフォルトフォントのステータスを一括設定する
...
extern	int			NS_FontCacheStringDrawToHandle(            int x, int y, const TCHAR *StrData, int Color, int EdgeColor, BASEIMAGE *DestImage, RECT *ClipRect /* NULL 可 */ , int FontHandle, int VerticalFlag = FALSE , SIZE *DrawSizeP = NULL ) ;
...
extern	int			NS_GetFontStateToHandle(                   TCHAR *FontName, int *Size, int *Thick, int FontHandle) ;										// フォントの情報を得る
...
// 基本イメージデータのロード＋ＤＩＢ関係
extern	int			NS_CreateGraphImageOrDIBGraph(    const TCHAR *FileName, void *DataImage, int DataImageSize, int DataImageType /* LOADIMAGE_TYPE_FILE 等 */ , int BmpFlag, int ReverseFlag, BASEIMAGE *Image, BITMAPINFO **BmpInfo, void **GraphData ) ;				// 登録されている各種グラフィックローダ関数から、ＢＭＰデータもしくは GraphImageデータを構築する
extern	int			NS_CreateGraphImageType2(         STREAMDATA *Src, BASEIMAGE *Dest ) ;																																													// 汎用読み込み処理によるグラフィックイメージ構築関数( 0:成功  -1:失敗 )
extern	int			NS_CreateBmpInfo(                 BITMAPINFO *BmpInfo, int Width, int Height, int Pitch, void *SrcGrData, void **DestGrData ) ;																														// フルカラー形式のBITMAPINFO構造体を作成する
extern	DWORD		NS_GetGraphImageFullColorCode(    BASEIMAGE *GraphImage, int x, int y ) ;																																												// BASEIMAGE 構造体の画像情報から指定の座標のフルカラーコードを取得する
extern	int			NS_CreateGraphImage_plus_Alpha(   const TCHAR *FileName, void *RgbImage, int RgbImageSize, int RgbImageType, void *AlphaImage, int AlphaImageSize, int AlphaImageType, BASEIMAGE *RgbGraphImage, BASEIMAGE *AlphaGraphImage, int ReverseFlag ) ;		// 各種グラフィックデータからグラフィックイメージデータとアルファマップ用イメージデータを構築する
extern	int			NS_ReverseGraphImage(             BASEIMAGE *GraphImage ) ;																																															// 指定の GraphImage を左右反転する
#ifdef __WINDOWS__
extern	HBITMAP		NS_CreateDIBGraph(                const TCHAR *FileName,                                                                                                                            int ReverseFlag,          COLORDATA *SrcColor) ;					// ＤＩＢグラフィックを作成する
extern	HBITMAP		NS_CreateDIBGraphToMem(           BITMAPINFO *BmpInfo, void *GraphData,                                                                                                             int ReverseFlag,          COLORDATA *SrcColor ) ;					// ＤＩＢグラフィックをメモリイメージから作成する
extern	int			NS_CreateDIBGraph_plus_Alpha(     const TCHAR *FileName, HBITMAP *RGBBmp, HBITMAP *AlphaBmp,                                                                                        int ReverseFlag = FALSE , COLORDATA *SrcColor = NULL ) ;			// ファイルからＤＩＢグラフィックとマスクグラフィックを作成する
extern	HBITMAP		NS_CreateDIBGraphVer2(            const TCHAR *FileName, void *MemImage, int MemImageSize,                                       int ImageType,                                     int ReverseFlag,          COLORDATA *SrcColor ) ;					// ＤＩＢグラフィックを作成する(バージョン２)
extern	int			NS_CreateDIBGraphVer2_plus_Alpha( const TCHAR *FileName, void *MemImage, int MemImageSize, void *AlphaImage, int AlphaImageSize, int ImageType, HBITMAP *RGBBmp, HBITMAP *AlphaBmp, int ReverseFlag,          COLORDATA *SrcColor ) ;					// ＤＩＢグラフィックを作成する
extern	int			NS_ConvBitmapToGraphImage(        BITMAPINFO *BmpInfo, void *GraphData, BASEIMAGE *GraphImage, int CopyFlag ) ;																																		// ＢＭＰ を GraphImage に変換する( Ret 0:正常終了  1:コピーを行った  -1:エラー )
extern	int			NS_ConvGraphImageToBitmap(        BASEIMAGE *GraphImage, BITMAPINFO *BmpInfo, void **GraphData, int CopyFlag, int FullColorConv = TRUE ) ;																												// GraphImage を ＢＭＰ に変換する(アルファデータはあっても無視される)( Ret 0:正常終了  1:コピーを行った  -1:エラー )
#endif // __WINDOWS__

//extern int		NS_AddUserGraphLoadFunction( int ( *UserLoadFunc )( FILE *fp, BITMAPINFO **BmpInfo, void **GraphData ) ) ;																					// ユーザー定義のグラフィックロード関数を登録する
//extern int		NS_AddUserGraphLoadFunction2( int ( *UserLoadFunc )( void *Image, int ImageSize, int ImageType, BITMAPINFO **BmpInfo, void **GraphData ) ) ;												// ユーザー定義のグラフィックロード関数を登録する
//extern int		NS_AddUserGraphLoadFunction3( int ( *UserLoadFunc )( void *DataImage, int DataImageSize, int DataImageType, int BmpFlag, BASEIMAGE *Image, BITMAPINFO **BmpInfo, void **GraphData ) ) ;	// ユーザー定義のグラフィックロード関数Ver3を登録する
extern	int			NS_AddUserGraphLoadFunction4( int (* UserLoadFunc )( STREAMDATA *Src, BASEIMAGE *Image ) ) ; 																								// ユーザー定義のグラフィックロード関数Ver4を登録する
//extern int		NS_SubUserGraphLoadFunction( int ( *UserLoadFunc )( FILE *fp, BITMAPINFO **BmpInfo, void **GraphData ) ) ;																					// ユーザー定義のグラフィックロード関数を登録から抹消する
//extern int		NS_SubUserGraphLoadFunction2( int ( *UserLoadFunc )( void *Image, int ImageSize, int ImageType, BITMAPINFO **BmpInfo, void **GraphData ) ) ;												// ユーザー定義のグラフィックロード関数を登録から抹消する
//extern int		NS_SubUserGraphLoadFunction3( int ( *UserLoadFunc )( void *DataImage, int DataImageSize, int DataImageType, int BmpFlag, BASEIMAGE *Image, BITMAPINFO **BmpInfo, void **GraphData ) ) ;	// ユーザー定義のグラフィックロード関数Ver3を登録から抹消する
extern	int			NS_SubUserGraphLoadFunction4( int (* UserLoadFunc )( STREAMDATA *Src, BASEIMAGE *Image ) ) ; 																								// ユーザー定義のグラフィックロード関数Ver4を登録から抹消する

extern	int			NS_SetUseFastLoadFlag(            int Flag ) ;														// 高速読み込みルーチンを使うか否かのフラグをセットする
extern	int			NS_GetGraphDataShavedMode(        void ) ;															// グラフィック減色時の画像劣化緩和処理モードの取得
extern	int			NS_SetGraphDataShavedMode(        int ShavedMode ) ;												// グラフィック減色時の画像劣化緩和処理モードの変更

//基本イメージデータ構造体関係
extern	int			NS_CreateBaseImage(               const TCHAR *FileName, void *FileImage, int FileImageSize, int DataType /*=LOADIMAGE_TYPE_FILE*/ , BASEIMAGE *BaseImage,  int ReverseFlag ) ;			// 各種グラフィックデータから基本イメージデータを構築する
extern	int			NS_CreateGraphImage(              const TCHAR *FileName, void *DataImage, int DataImageSize, int DataImageType,                      BASEIMAGE *GraphImage, int ReverseFlag ) ;			// CreateBaseImage の旧名称
...
extern	int			NS_CreateBaseImageToMem(                                 void *FileImage, int FileImageSize,                                         BASEIMAGE *BaseImage,  int ReverseFlag = FALSE ) ;	// メモリ上に展開された画像ファイルから基本イメージデータを構築する
extern	int			NS_CreateARGB8ColorBaseImage(     int SizeX, int SizeY,                       BASEIMAGE *BaseImage ) ;																						// ＡＲＧＢ８カラーの空の基本イメージデータを作成する
extern	int			NS_CreateXRGB8ColorBaseImage(     int SizeX, int SizeY,                       BASEIMAGE *BaseImage ) ;																						// ＸＲＧＢ８カラーの空の基本イメージデータを作成する
extern	int			NS_CreateRGB8ColorBaseImage(      int SizeX, int SizeY,                       BASEIMAGE *BaseImage ) ;																						// ＲＧＢ８カラーの空の基本イメージデータを作成する
extern	int			NS_CreateARGB4ColorBaseImage(     int SizeX, int SizeY,                       BASEIMAGE *BaseImage ) ;																						// ＡＲＧＢ４カラーの空の基本イメージデータを作成する
extern	int			NS_CreatePAL8ColorBaseImage(      int SizeX, int SizeY,                       BASEIMAGE *BaseImage ) ;																						// パレット８ビットカラーの空の基本イメージデータを作成する
extern	int			NS_CreateColorDataBaseImage(      int SizeX, int SizeY, COLORDATA *ColorData, BASEIMAGE *BaseImage ) ;																						// 指定のカラーフォーマットの殻の基本イメージデータを作成する
extern	int			NS_GetBaseImageGraphDataSize(     BASEIMAGE *BaseImage ) ;																																	// 基本イメージデータのイメージサイズを取得する( byte単位 )
...
extern	int			NS_ConvertNormalFormatBaseImage(  BASEIMAGE *BaseImage ) ;																																	// DX_BASEIMAGE_FORMAT_NORMAL 以外の形式のイメージデータを DX_BASEIMAGE_FORMAT_NORMAL 形式のイメージに変換する

extern	int			NS_GetDrawScreenBaseImage(        int x1, int y1, int x2, int y2, BASEIMAGE *BaseImage ) ;																									// 描画対象の画面から指定領域のグラフィックを基本イメージデータに転送する
#ifdef __WINDOWS__
extern	int			NS_UpdateLayerdWindowForBaseImage(                   BASEIMAGE *BaseImage ) ;																												// 基本イメージを使用して UpdateLayerdWindow を行う
extern	int			NS_UpdateLayerdWindowForPremultipliedAlphaBaseImage( BASEIMAGE *BaseImage ) ;																												// 乗算済みアルファの基本イメージを使用して UpdateLayerdWindow を行う
#endif // __WINDOWS__
...
extern	int			NS_GetPaletteBaseImage(           BASEIMAGE *BaseImage, int PaletteNo, int *r, int *g, int *b, int *a ) ;																					// 基本イメージデータのパレットを取得する
...
extern	int			NS_GetPixelPalCodeBaseImage(      BASEIMAGE *BaseImage, int x, int y ) ;																													// 基本イメージデータの指定の座標の色コードを取得する(パレット画像用)
...
extern	int			NS_BltBaseImage(                  int SrcX, int SrcY, int SrcSizeX, int SrcSizeY, int DestX, int DestY, BASEIMAGE *SrcBaseImage, BASEIMAGE *DestBaseImage ) ;								// 基本イメージデータを転送する
extern	int			NS_BltBaseImage(                                                                  int DestX, int DestY, BASEIMAGE *SrcBaseImage, BASEIMAGE *DestBaseImage ) ;								// 基本イメージデータを転送する
extern	int			NS_BltBaseImageWithTransColor(    int SrcX, int SrcY, int SrcSizeX, int SrcSizeY, int DestX, int DestY, BASEIMAGE *SrcBaseImage, BASEIMAGE *DestBaseImage, int Tr, int Tg, int Tb, int Ta ) ;	// 基本イメージデータを透過色処理付きで転送する
extern	int			NS_BltBaseImageWithAlphaBlend(    int SrcX, int SrcY, int SrcSizeX, int SrcSizeY, int DestX, int DestY, BASEIMAGE *SrcBaseImage, BASEIMAGE *DestBaseImage, int Opacity = 255 ) ;			// 基本イメージデータをアルファ値のブレンドを考慮した上で転送する( Opacity は透明度 : 0( 完全透明 ) ～ 255( 完全不透明 ) )( 出力先が ARGB8 形式以外の場合はエラーになります )
extern	int			NS_ReverseBaseImageH(             BASEIMAGE *BaseImage ) ;																																	// 基本イメージデータの左右を反転する
extern	int			NS_ReverseBaseImageV(             BASEIMAGE *BaseImage ) ;																																	// 基本イメージデータの上下を反転する
extern	int			NS_ReverseBaseImage(              BASEIMAGE *BaseImage ) ;																																	// 基本イメージデータの上下左右を反転する
extern	int			NS_CheckPixelAlphaBaseImage(      BASEIMAGE *BaseImage ) ;																																	// 基本イメージデータに含まれるピクセルのアルファ値をチェックする( 戻り値   -1:エラー  0:画像にアルファ成分が無い  1:画像にアルファ成分があり、すべて最大(255)値  2:画像にアルファ成分があり、存在するアルファ値は最小(0)と最大(255)もしくは最小(0)のみ　3:画像にアルファ成分があり、最小と最大以外の中間の値がある )  

#ifndef DX_NON_SAVEFUNCTION

extern	int			NS_SaveBaseImageToBmp(            const TCHAR *FilePath, BASEIMAGE *BaseImage ) ;																											// 基本画像データをＢＭＰ画像として保存する
#ifndef DX_NON_PNGREAD
extern	int			NS_SaveBaseImageToPng(            const TCHAR *FilePath, BASEIMAGE *BaseImage, int CompressionLevel ) ;																					// 基本画像データをＰＮＧ画像として保存する
#endif
#ifndef DX_NON_JPEGREAD
extern	int			NS_SaveBaseImageToJpeg(           const TCHAR *FilePath, BASEIMAGE *BaseImage, int Quality, int Sample2x1 ) ;																				// 基本画像データをＪＰＥＧ画像として保存する
#endif
...
extern	DWORD		NS_GetColor(               int Red, int Green, int Blue ) ;														// ３原色値から現在の画面モードに対応した色データ値を得る
extern	int			NS_GetColor2(              int Color, int *Red, int *Green, int *Blue ) ;											// 画面モードに対応した色データ値から個々の３原色データを抜き出す
extern	int			NS_GetColor3(              COLORDATA *ColorData, int Red, int Green, int Blue, int Alpha = 255 ) ;					// ３原色値から指定のピクセルフォーマットに対応した色データ値を得る
extern	int			NS_GetColor4(              COLORDATA *DestColorData, COLORDATA* SrcColorData, int SrcColor ) ;						// ２つのカラーフォーマット間のデータ変換を行った情報を得る 
extern	int			NS_GetColor5(              COLORDATA *ColorData, int Color, int *Red, int *Green, int *Blue, int *Alpha = NULL ) ;	// 指定カラーフォーマットに対応した色データ値から個々の３原色データを抜き出す
extern	int			NS_CreatePaletteColorData( COLORDATA *ColorDataBuf ) ;																// パレットカラーのカラー情報を構築する
extern	int			NS_CreateXRGB8ColorData(   COLORDATA *ColorDataBuf ) ;																// ＸＲＧＢ８カラーのカラー情報を構築する
extern	int			NS_CreateARGB8ColorData(   COLORDATA *ColorDataBuf ) ;																// ＡＲＧＢ８カラーのカラー情報を構築する
extern	int			NS_CreateARGB4ColorData(   COLORDATA *ColorDataBuf ) ;																// ＡＲＧＢ４カラーのカラー情報を構築する
extern	int			NS_CreateFullColorData(    COLORDATA *ColorDataBuf ) ;																// フルカラーＤＩＢのカラー情報を構築する
extern	int			NS_CreateGrayColorData(    COLORDATA *ColorDataBuf ) ;																// グレースケールのカラー情報を構築する
extern	int			NS_CreatePal8ColorData(    COLORDATA *ColorDataBuf ) ;																// パレット２５６色のカラー情報を構築する
extern	int			NS_CreateColorData(        COLORDATA *ColorDataBuf, int ColorBitDepth,
									        ...
									        int ChannelNum = 0, int ChannelBitDepth = 0, int FloatTypeFlag = FALSE ) ;				// カラーデータを作成する
extern	void		NS_SetColorDataNoneMask(   COLORDATA *ColorData ) ;																// NoneMask 以外の要素を埋めた COLORDATA 構造体の情報を元に NoneMask をセットする
extern	int			NS_CmpColorData(           COLORDATA *ColorData1, COLORDATA *ColorData2 ) ;										// 二つのカラーデータが等しいかどうか調べる( TRUE:等しい  FALSE:等しくない )
...
extern	int			NS_LoadSoftImageToMem(              void *FileImage, int FileImageSize ) ;												// ソフトウエアで扱うイメージのメモリからの読み込み( -1:エラー  -1以外:イメージハンドル )
...
#ifdef __WINDOWS__
extern	int			NS_UpdateLayerdWindowForSoftImage(                   int SIHandle ) ;													// ソフトウエアで扱うイメージを使用して UpdateLayerdWindow を行う( イメージはウインドウのクライアント領域と同じサイズである必要があります )
extern	int			NS_UpdateLayerdWindowForPremultipliedAlphaSoftImage( int SIHandle ) ;													// 乗算済みアルファのソフトウエアで扱うイメージを使用して UpdateLayerdWindow を行う( イメージはウインドウのクライアント領域と同じサイズである必要があります )
#endif // __WINDOWS__
...
// サウンドデータ管理系関数
extern	int			NS_InitSoundMem(                        int LogOutFlag = FALSE ) ;																	// メモリに読みこんだWAVEデータを削除し、初期化する

extern	int			NS_AddSoundData(                        int Handle = -1 ) ;																										// 新しいサウンドデータ領域を確保する
extern	int			NS_AddStreamSoundMem(                   STREAMDATA *Stream, int LoopNum,  int SoundHandle, int StreamDataType, int *CanStreamCloseFlag, int UnionHandle = -1 ) ;	// ストリーム風サウンドデータにサウンドデータを追加する
extern	int			NS_AddStreamSoundMemToMem(              void *FileImage, int FileImageSize, int LoopNum,  int SoundHandle, int StreamDataType, int UnionHandle = -1 ) ;			// ストリーム風サウンドデータにサウンドデータを追加する
extern	int			NS_AddStreamSoundMemToFile(             const TCHAR *WaveFile, int LoopNum,  int SoundHandle, int StreamDataType, int UnionHandle = -1 ) ;							// ストリーム風サウンドデータにサウンドデータを追加する
extern	int			NS_SetupStreamSoundMem(                 int SoundHandle ) ;																		// ストリーム風サウンドデータの再生準備を行う
extern	int			NS_PlayStreamSoundMem(                  int SoundHandle, int PlayType = DX_PLAYTYPE_LOOP , int TopPositionFlag = TRUE ) ;			// ストリーム風サウンドデータの再生開始
extern	int			NS_CheckStreamSoundMem(                 int SoundHandle ) ;																		// ストリーム風サウンドデータの再生状態を得る
extern	int			NS_StopStreamSoundMem(                  int SoundHandle ) ;																		// ストリーム風サウンドデータの再生終了
...
extern	int			NS_ProcessStreamSoundMem(               int SoundHandle ) ;																		// ストリームサウンドの再生処理関数
extern	int			NS_ProcessStreamSoundMemAll(            void ) ;																					// 有効なストリームサウンドのすべて再生処理関数にかける


extern	int			NS_LoadSoundMem2(                       const TCHAR *FileName1, const TCHAR *FileName2 ) ;											// 前奏部とループ部に分かれたサウンドデータの作成
extern	int			NS_LoadBGM(                             const TCHAR *FileName ) ;																	// 主にＢＧＭを読み込むのに適した関数

extern	int			NS_LoadSoundMemBase(                    const TCHAR *FileName, int BufferNum,      int UnionHandle = -1 ) ;						// サウンドデータを追加する
extern	int			NS_LoadSoundMem(                        const TCHAR *FileName, int BufferNum = 3 , int UnionHandle = -1 ) ;						// サウンドデータを追加する
extern	int			NS_LoadSoundMemToBufNumSitei(           const TCHAR *FileName, int BufferNum ) ;													// 同時再生数指定型サウンド追加関数
extern	int			NS_LoadSoundMemByResource(              const TCHAR *ResourceName, const TCHAR *ResourceType, int BufferNum = 1 ) ;				// サウンドをリソースから読み込む
extern	int			NS_DuplicateSoundMem(                   int SrcSoundHandle, int BufferNum = 3 ) ;													// 同じサウンドデータを使用するサウンドハンドルを作成する( DX_SOUNDDATATYPE_MEMNOPRESS タイプのみ )

extern	int			NS_LoadSoundMemByMemImageBase(          void *FileImage, int FileImageSize, int BufferNum, int UnionHandle = -1 ) ;				// メモリ上に展開されたファイルイメージからハンドルを作成する(ベース関数)
extern	int			NS_LoadSoundMemByMemImage(              void *FileImage, int FileImageSize,                int UnionHandle = -1 ) ;				// メモリ上に展開されたファイルイメージからハンドルを作成する(バッファ数指定あり) 
extern	int			NS_LoadSoundMemByMemImage2(             void *WaveImage, int WaveImageSize, WAVEFORMATEX *WaveFormat, int WaveHeaderSize ) ;		// メモリ上に展開されたファイルイメージからハンドルを作成する2
extern	int			NS_LoadSoundMemByMemImageToBufNumSitei( void *FileImage, int FileImageSize, int BufferNum ) ;										// メモリ上に展開されたファイルイメージからハンドルを作成する
extern	int			NS_LoadSoundMem2ByMemImage(             void *FileImage1, int FileImageSize1, void *FileImage2, int FileImageSize2 ) ;				// メモリ上に展開されたファイルイメージから前奏部とループ部に分かれたハンドルを作成する
extern	int			NS_LoadSoundMemFromSoftSound(           int SoftSoundHandle, int BufferNum = 3 ) ;													// ソフトウエアで波形を制御するサウンドデータからサウンドハンドルを作成する

extern	int			NS_DeleteSoundMem(                      int SoundHandle, int LogOutFlag = FALSE ) ;												// メモリに読み込んだWAVEデータを削除する

extern	int			NS_PlaySoundMem(                        int SoundHandle, int PlayType, int TopPositionFlag = TRUE ) ;								// メモリに読みこんだWAVEデータを再生する
extern	int			NS_StopSoundMem(                                                     int SoundHandle ) ;											// メモリに読み込んだWAVEデータの再生を止める
extern	int			NS_CheckSoundMem(                                                    int SoundHandle ) ;											// メモリに読みこんだWAVEデータが再生中か調べる
extern	int			NS_SetPanSoundMem(                      int PanPal,                  int SoundHandle ) ;											// メモリに読みこんだWAVEデータの再生にパンを設定する
extern	int			NS_GetPanSoundMem(                                                   int SoundHandle ) ;											// メモリに読みこんだWAVEデータの再生にパンを取得する
extern	int			NS_SetVolumeSoundMem(                   int VolumePal,               int SoundHandle ) ;											// メモリに読みこんだWAVEデータの再生にボリュームを設定する( 100分の1デシベル単位 )
extern	int			NS_ChangeVolumeSoundMem(                int VolumePal,               int SoundHandle ) ;											// メモリに読みこんだWAVEデータの再生にボリュームを設定する( パーセント指定 )
extern	int			NS_GetVolumeSoundMem(                                                int SoundHandle ) ;											// メモリに読みこんだWAVEデータの再生のボリュームを取得する
extern	int			NS_SetFrequencySoundMem(                int FrequencyPal,            int SoundHandle ) ;											// メモリに読み込んだWAVEデータの再生周波数を設定する
extern	int			NS_GetFrequencySoundMem(                                             int SoundHandle ) ;											// メモリに読み込んだWAVEデータの再生周波数を取得する
extern	int			NS_ResetFrequencySoundMem(                                           int SoundHandle ) ;											// メモリに読み込んだWAVEデータの再生周波数を読み込み直後の状態に戻す

extern	int			NS_SetNextPlayPanSoundMem(              int PanPal,                  int SoundHandle ) ;											// メモリに読みこんだWAVEデータの次の再生にのみ使用するパンを設定する
extern	int			NS_SetNextPlayVolumeSoundMem(           int VolumePal,               int SoundHandle ) ;											// メモリに読みこんだWAVEデータの次の再生にのみ使用するボリュームを設定する( 100分の1デシベル単位 )
extern	int			NS_ChangeNextPlayVolumeSoundMem(        int VolumePal,               int SoundHandle ) ;											// メモリに読みこんだWAVEデータの次の再生にのみ使用するボリュームを設定する( パーセント指定 )
extern	int			NS_SetNextPlayFrequencySoundMem(        int FrequencyPal,            int SoundHandle ) ;											// メモリに読み込んだWAVEデータの次の再生にのみ使用する再生周波数を設定する

extern	int			NS_SetCurrentPositionSoundMem(          int SamplePosition,          int SoundHandle ) ;											// サウンドハンドルの再生位置をサンプル単位で変更する(再生が止まっている時のみ有効)
extern	int			NS_GetCurrentPositionSoundMem(                                       int SoundHandle ) ;											// サウンドハンドルの再生位置をサンプル単位で取得する
extern	int			NS_SetSoundCurrentPosition(             int Byte,                    int SoundHandle ) ;											// サウンドハンドルの再生位置をバイト単位で変更する(再生が止まっている時のみ有効)
...
extern	int			NS_GetSoundTotalSample(                                              int SoundHandle ) ;											// サウンドハンドルの音の総時間を取得する(単位はサンプル)
extern	int			NS_GetSoundTotalTime(                                                int SoundHandle ) ;											// サウンドハンドルの音の総時間を取得する(単位はミリ秒)

extern	int			NS_SetLoopPosSoundMem(                  int LoopTime,                int SoundHandle ) ;											// サウンドハンドルにループ位置を設定する
extern	int			NS_SetLoopTimePosSoundMem(              int LoopTime,                int SoundHandle ) ;											// サウンドハンドルにループ位置を設定する
extern	int			NS_SetLoopSamplePosSoundMem(            int LoopSamplePosition,      int SoundHandle ) ;											// サウンドハンドルにループ位置を設定する

extern	int			NS_SetLoopStartTimePosSoundMem(         int LoopStartTime,           int SoundHandle ) ;											// サウンドハンドルにループ開始位置を設定する
extern	int			NS_SetLoopStartSamplePosSoundMem(       int LoopStartSamplePosition, int SoundHandle ) ;											// サウンドハンドルにループ開始位置を設定する
...
//extern	int			NS_Set3DInnerRadiusSoundMem(            float Radius,                                   int SoundHandle ) ;						// サウンドハンドルの３Ｄサウンド用の音量１００％で聞こえる距離を設定する
extern	int			NS_Set3DVelocitySoundMem(               VECTOR Velocity,                                int SoundHandle ) ;						// サウンドハンドルの３Ｄサウンド用の移動速度を設定する
//extern	int			NS_Set3DFrontPosition_UpVecYSoundMem(   VECTOR FrontPosition,                           int SoundHandle ) ;						// サウンドハンドルの３Ｄサウンド用の方向を決定するための前方座標を設定する( 音源の上方向はY軸固定 )
//extern	int			NS_Set3DFrontPositionSoundMem(          VECTOR FrontPosition, VECTOR UpVector,          int SoundHandle ) ;						// サウンドハンドルの３Ｄサウンド用の方向を決定するための前方座標と上方向を設定する
//extern	int			NS_Set3DConeAngleSoundMem(              float InnerAngle, float OuterAngle,             int SoundHandle ) ;						// サウンドハンドルの３Ｄサウンド用の方向の角度範囲を設定する
//extern	int			NS_Set3DConeVolumeSoundMem(             float InnerAngleVolume, float OuterAngleVolume, int SoundHandle ) ;						// サウンドハンドルの３Ｄサウンド用の方向の角度範囲の音量倍率を設定する

// 設定関係関数
extern	int			NS_SetCreateSoundDataType(              int SoundDataType ) ;																		// 作成するサウンドのデータ形式を設定する( DX_SOUNDDATATYPE_MEMNOPRESS 等 )
extern	int			NS_GetCreateSoundDataType(              void ) ;																					// 作成するサウンドのデータ形式を取得する( DX_SOUNDDATATYPE_MEMNOPRESS 等 )
...
extern	int			NS_SetCreate3DSoundFlag(                    int Flag ) ;																			// 次に作成するサウンドを３Ｄサウンド用にするかどうかを設定する( TRUE:３Ｄサウンド用にする  FALSE:３Ｄサウンド用にしない( デフォルト ) )
extern	int			NS_Set3DSoundOneMetre(                      float Distance ) ;																		// ３Ｄ空間の１メートルに当る距離を設定する( デフォルト:1.0f )
extern	int			NS_Set3DSoundListenerPosAndFrontPos_UpVecY( VECTOR Position, VECTOR FrontPosition ) ;												// ３Ｄサウンドのリスナーの位置とリスナーの前方位置を設定する( リスナーの上方向位置はＹ軸固定 )
...
// 情報取得系関数
extern	void*		NS_GetDSoundObj(                        void ) ;	/* 戻り値を IDirectSound * にキャストして下さい */								// ＤＸライブラリが使用している DirectSound オブジェクトを取得する
...
// ラッパー関数
extern	int			NS_PlaySoundFile(						const TCHAR *FileName, int PlayType ) ;														// WAVEファイルを再生する
extern	int			NS_PlaySound(							const TCHAR *FileName, int PlayType ) ;														// PlaySoundFile の旧名称
extern	int			NS_CheckSoundFile(						void ) ;																					// WAVEファイルの再生中か調べる
extern	int			NS_CheckSound(							void ) ;																					// CheckSoundFile の旧名称
extern	int			NS_StopSoundFile(						void ) ;																					// WAVEファイルの再生を止める
extern	int			NS_StopSound(							void ) ;																					// StopSoundFile の旧名称
extern	int			NS_SetVolumeSoundFile(					int VolumePal ) ;																			// WAVEファイルの音量をセットする
extern	int			NS_SetVolumeSound(						int VolumePal ) ;																			// SetVolumeSound の旧名称

// ソフトウエア制御サウンド系関数
extern	int			NS_InitSoftSound(						void ) ;																					// ソフトウエアで扱う波形データをすべて解放する
extern	int			NS_LoadSoftSound(						const TCHAR *FileName ) ;																	// ソフトウエアで扱う波形データをファイルから作成する
extern	int			NS_LoadSoftSoundFromMemImage(			const void *FileImage, int FileImageSize ) ;												// ソフトウエアで扱う波形データをメモリ上に展開されたファイルイメージから作成する
extern	int			NS_MakeSoftSound(						int UseFormat_SoftSoundHandle, int SampleNum ) ;											// ソフトウエアで扱う空の波形データを作成する( フォーマットは引数のソフトウエアサウンドハンドルと同じものにする )
extern	int			NS_MakeSoftSound2Ch16Bit44KHz(			int SampleNum ) ;																			// ソフトウエアで扱う空の波形データを作成する( チャンネル数:2 量子化ビット数:16bit サンプリング周波数:44KHz )
extern	int			NS_MakeSoftSound2Ch16Bit22KHz(			int SampleNum ) ;																			// ソフトウエアで扱う空の波形データを作成する( チャンネル数:2 量子化ビット数:16bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSound2Ch8Bit44KHz(			int SampleNum ) ;																			// ソフトウエアで扱う空の波形データを作成する( チャンネル数:2 量子化ビット数:8bit サンプリング周波数:44KHz )
extern	int			NS_MakeSoftSound2Ch8Bit22KHz(			int SampleNum ) ;																			// ソフトウエアで扱う空の波形データを作成する( チャンネル数:2 量子化ビット数:8bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSound1Ch16Bit44KHz(			int SampleNum ) ;																			// ソフトウエアで扱う空の波形データを作成する( チャンネル数:1 量子化ビット数:16bit サンプリング周波数:44KHz )
extern	int			NS_MakeSoftSound1Ch16Bit22KHz(			int SampleNum ) ;																			// ソフトウエアで扱う空の波形データを作成する( チャンネル数:1 量子化ビット数:16bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSound1Ch8Bit44KHz(			int SampleNum ) ;																			// ソフトウエアで扱う空の波形データを作成する( チャンネル数:1 量子化ビット数:8bit サンプリング周波数:44KHz )
extern	int			NS_MakeSoftSound1Ch8Bit22KHz(			int SampleNum ) ;																			// ソフトウエアで扱う空の波形データを作成する( チャンネル数:1 量子化ビット数:8bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSoundCustom(				int ChannelNum, int BitsPerSample, int SamplesPerSec, int SampleNum );						// ソフトウエアで扱う空の波形データを作成する
extern	int			NS_DeleteSoftSound(					int SoftSoundHandle ) ;																		// ソフトウエアで扱う波形データを解放する
#ifndef DX_NON_SAVEFUNCTION
extern	int			NS_SaveSoftSound(						int SoftSoundHandle, const TCHAR *FileName ) ;												// ソフトウエアで扱う波形データを無圧縮Wav形式で保存する
#endif // DX_NON_SAVEFUNCTION
extern	int			NS_GetSoftSoundSampleNum(				int SoftSoundHandle ) ;																		// ソフトウエアで扱う波形データのサンプル数を取得する
extern	int			NS_GetSoftSoundFormat(					int SoftSoundHandle, int *Channels, int *BitsPerSample, int *SamplesPerSec ) ;				// ソフトウエアで扱う波形データのフォーマットを取得する
extern	int			NS_ReadSoftSoundData(					int SoftSoundHandle, int SamplePosition, int *Channel1, int *Channel2 ) ;					// ソフトウエアで扱う波形データのサンプルを読み取る
extern	int			NS_WriteSoftSoundData(					int SoftSoundHandle, int SamplePosition, int Channel1, int Channel2 ) ;						// ソフトウエアで扱う波形データのサンプルを書き込む
extern	void		*NS_GetSoftSoundDataImage(				int SoftSoundHandle ) ;																		// ソフトウエアで扱う波形データの波形イメージが格納されているメモリアドレスを取得する

extern	int			NS_InitSoftSoundPlayer(				void ) ;																					// ソフトウエアで扱う波形データのプレイヤーをすべて解放する
extern	int			NS_MakeSoftSoundPlayer(				int UseFormat_SoftSoundHandle ) ;															// ソフトウエアで扱う波形データのプレイヤーを作成する( フォーマットは引数のソフトウエアサウンドハンドルと同じものにする )
extern	int			NS_MakeSoftSoundPlayer2Ch16Bit44KHz(	void ) ;																					// ソフトウエアで扱う波形データのプレイヤーを作成する( チャンネル数:2 量子化ビット数:16bit サンプリング周波数:44KHz )
extern	int			NS_MakeSoftSoundPlayer2Ch16Bit22KHz(	void ) ;																					// ソフトウエアで扱う波形データのプレイヤーを作成する( チャンネル数:2 量子化ビット数:16bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSoundPlayer2Ch8Bit44KHz(	void ) ;																					// ソフトウエアで扱う波形データのプレイヤーを作成する( チャンネル数:2 量子化ビット数:8bit サンプリング周波数:44KHz )
extern	int			NS_MakeSoftSoundPlayer2Ch8Bit22KHz(	void ) ;																					// ソフトウエアで扱う波形データのプレイヤーを作成する( チャンネル数:2 量子化ビット数:8bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSoundPlayer1Ch16Bit44KHz(	void ) ;																					// ソフトウエアで扱う波形データのプレイヤーを作成する( チャンネル数:1 量子化ビット数:16bit サンプリング周波数:44KHz )
extern	int			NS_MakeSoftSoundPlayer1Ch16Bit22KHz(	void ) ;																					// ソフトウエアで扱う波形データのプレイヤーを作成する( チャンネル数:1 量子化ビット数:16bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSoundPlayer1Ch8Bit44KHz(	void ) ;																					// ソフトウエアで扱う波形データのプレイヤーを作成する( チャンネル数:1 量子化ビット数:8bit サンプリング周波数:44KHz )
extern	int			NS_MakeSoftSoundPlayer1Ch8Bit22KHz(	void ) ;																					// ソフトウエアで扱う波形データのプレイヤーを作成する( チャンネル数:1 量子化ビット数:8bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSoundPlayerCustom(			int ChannelNum, int BitsPerSample, int SamplesPerSec ) ;									// ソフトウエアで扱う波形データのプレイヤーを作成する
extern	int			NS_DeleteSoftSoundPlayer(				int SSoundPlayerHandle ) ;																	// ソフトウエアで扱う波形データのプレイヤーを削除する
extern	int			NS_AddDataSoftSoundPlayer(				int SSoundPlayerHandle, int SoftSoundHandle, int AddSamplePosition, int AddSampleNum ) ;	// ソフトウエアで扱う波形データのプレイヤーに波形データを追加する( フォーマットが同じではない場合はエラー )
extern	int			NS_AddDirectDataSoftSoundPlayer(		int SSoundPlayerHandle, void *SoundData, int AddSampleNum ) ;								// ソフトウエアで扱う波形データのプレイヤーにプレイヤーが対応したフォーマットの生波形データを追加する
extern	int			NS_AddOneDataSoftSoundPlayer(			int SSoundPlayerHandle, int Channel1, int Channel2 ) ;										// ソフトウエアで扱う波形データのプレイヤーに波形データを一つ追加する
extern	int			NS_GetSoftSoundPlayerFormat(			int SSoundPlayerHandle, int *Channels, int *BitsPerSample, int *SamplesPerSec ) ;			// ソフトウエアで扱う波形データのプレイヤーが扱うデータフォーマットを取得する
extern	int			NS_StartSoftSoundPlayer(				int SSoundPlayerHandle ) ;																	// ソフトウエアで扱う波形データのプレイヤーの再生処理を開始する
extern	int			NS_CheckStartSoftSoundPlayer(			int SSoundPlayerHandle ) ;																	// ソフトウエアで扱う波形データのプレイヤーの再生処理が開始されているか取得する( TRUE:開始している  FALSE:停止している )
extern	int			NS_StopSoftSoundPlayer(				int SSoundPlayerHandle ) ;																	// ソフトウエアで扱う波形データのプレイヤーの再生処理を停止する
extern	int			NS_ResetSoftSoundPlayer(				int SSoundPlayerHandle ) ;																	// ソフトウエアで扱う波形データのプレイヤーの状態を初期状態に戻す( 追加された波形データは削除され、再生状態だった場合は停止する )
extern	int			NS_GetStockDataLengthSoftSoundPlayer(	int SSoundPlayerHandle ) ;																	// ソフトウエアで扱う波形データのプレイヤーに追加した波形データでまだ再生用サウンドバッファに転送されていない波形データのサンプル数を取得する
extern	int			NS_CheckSoftSoundPlayerNoneData(		int SSoundPlayerHandle ) ;																	// ソフトウエアで扱う波形データのプレイヤーに再生用サウンドバッファに転送していない波形データが無く、再生用サウンドバッファにも無音データ以外無いかどうかを取得する( TRUE:無音データ以外無い  FALSE:有効データがある )
...
extern	int			NS_LoadMusicMemByMemImage(				void *FileImage, int FileImageSize ) ;														// メモリ上に展開されたＭＩＤＩファイルを読み込む
...
extern	int			NS_PlayMusicByMemImage(				void *FileImage, int FileImageSize, int PlayType ) ;										// メモリ上に展開されているＭＩＤＩファイルを演奏する
