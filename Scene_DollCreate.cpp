...
bool Scene_DollCreate::Initialize(bool fSkipFrame){
	...
	for(int t=0; t<DOLL_GROUP_NUM; t++){
		for(int n=0; n<DOLL_TYPE_PER_GROUP; n++){
			if(!g_aliceInfo.data.dollRecepe[t*DOLL_TYPE_PER_GROUP+n+1]){
...
bool Scene_DollCreate::InitWindow(){
	...
	w_yesno.Setup(
		...
		true, ALIGN_CENTER);
	w_yesno.SetTitle(_T("これで人形を作成しますか？"));
	w_yesno.SetAllColor(
...
int Scene_DollCreate::Update(){
	...
	if(!SceneIsReserved()){
		...
		case SCNSTATE_DOLLCREATE_CHOOSESTONE:
			switch(s_attr.Move()){
			case SELECT_CHOOSE:
				if(g_aliceInfo.data.pieceNum[s_group.index][s_attr.index] > 0){
					// 引き続き属性の石を選ぶ
...
void Scene_DollCreate::Draw(){
	...
	for(int n=0; n<DOLL_TYPE_PER_GROUP; n++){
		...
		if(g_aliceInfo.data.dollRecepe[index]){
...
void Scene_DollCreate::DrawEachGroup(int x, int y, BYTE group, bool focused){
	...
	for(BYTE a=0; a<DOLL_ATTR_NUM-1; a++){
		...
		// 持っている欠片の個数の描画
		n = g_aliceInfo.data.pieceNum[group-1][a];
		DrawNum(n,
...
void Scene_DollCreate::SetAttrStone(BYTE group, BYTE attr, int pos){
	attrStones[pos] = attr;
	g_aliceInfo.data.pieceNum[group][attr-1]--;
}
...
void Scene_DollCreate::ResetAttrStone(int pos){
	...
	if(attr != DOLL_ATTR_NONE){
		// 該当する欠片の数を増やす
		g_aliceInfo.data.pieceNum[s_group.index][attr-1]++;
		// 欠片を回収する
...
void Scene_DollCreate::RegisterNewDoll(){
	...
	// シリアルナンバー
	g_temp.dollData.SetSerial(g_aliceInfo.GetSerial());
	// 属性ストーン
	for(int n=0; n<DOLL_ATTRSTONE_CREATE; n++){
